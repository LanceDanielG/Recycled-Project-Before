<?php
    ...
    $result = $connector->select("messages", "link like '%" . $_POST['messageUser'] . "%';");
    $msgs = array();
    while ($row = $result->fetch_assoc()) {
        $user = explode("-", $row['link']);
        $user = $user[0] == $_SESSION['username'] ? True : False;
        array_push($msgs, array($user ,$row['message']));
    }
?>
        ...
        <div id="layoutSidenav">
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid mt-4 mx-0 px-0">
                        <div class="row px-0 mx-0">
                            <h1 id="activeReceiver"> <?php echo $_POST['messageUser']; ?> </h1>
                        </div>
                        <div class="row container-fluid px-0 mx-0">
                            <?php
                                foreach ($msgs as $thing) {
                                    $temp = !$thing[0] ? "text-align: right;" : "";
                                    echo "<div class='row container-fluid border border-1 mx-0 px-0' style='". $temp . "'>
                                            <div class='col'>
                                                <p class='my-2 text-muted'>" .$thing[1] ."</p>
                                            </div>
                                        </div>";
                                }
                            ?>
                ...
                <form class="container" action="sendmsg.php" method="post" style="Position: absolute; bottom:5%;">
                    <input class="col-2" type="hidden" name="receiver" placeholder="<?php echo $_SESSION['username']; ?>" value="<?php  echo $_POST['messageUser']; ?>" required/>
